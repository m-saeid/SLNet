...
def load_scanobjectnn_data(partition):
    #download()
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'dataset/scanobject/h5_files')

    all_data = []
    all_label = []

    h5_name = DATA_DIR + '/main_split/' + partition + '_objectdataset_augmentedrot_scale75.h5'
...
class ScanObjectNN(Dataset):
    def __init__(self, num_points, partition='training', in_d=3, out_d=16):

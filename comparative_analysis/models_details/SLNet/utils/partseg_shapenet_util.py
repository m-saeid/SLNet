...
from utils.util import to_categorical, compute_overall_iou, log_experiment, IOStream
...
def init(args):
    ...
    shutil.copy('tasks/partseg_shapenet.py', f'{code_dir}/partseg_shapenet.py')
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=64, metavar='batch_size',     # 32>64
                        help='Size of batch)')
    parser.add_argument('--test_batch_size', type=int, default=32, metavar='batch_size',    # 32>64
                        help='Size of batch)')
...
def train(args, io):
    ...
    # =========== Dataloader =================
    train_data = PartNormalDataset(npoints=2048, split='trainval', normalize=False, out_d=args.embed_dim)
    print("The number of training data is:%d", len(train_data))

    test_data = PartNormalDataset(npoints=2048, split='test', normalize=False, out_d=args.embed_dim)
    ...
    for epoch in range(args.epochs):
        ...
        train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io, args)
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io, args):
    train_loss = 0.0
    count = 0.0
    accuracy = []
    shape_ious = 0.0
    ...
    for batch_id, (points, feature, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # target: b,n
        seg_pred = model(points, feature, norm_plt, to_categorical(label, num_classes))[0]  # seg_pred: b,n,50
...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, feature, label, target, norm_plt) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        batch_size, num_point, _ = points.size()
        points, feature, label, target, norm_plt = Variable(points.float()), Variable(feature.float()), Variable(label.long()), \
                                                   Variable(target.long()), Variable(norm_plt.float())
        points = points.transpose(2, 1)
        ...
        seg_pred = model(points, feature, norm_plt, to_categorical(label, num_classes))[0]  # b,n,50
...
def test(args, io):
    ...
    for batch_id, (points, feature, label, target, norm_plt) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        ...
        norm_plt = norm_plt.transpose(2, 1)
        points, feature, label, target, norm_plt = points.cuda(non_blocking=True), feature.cuda(non_blocking=True), \
                                                   label.squeeze().cuda(non_blocking=True), target.cuda(non_blocking=True), norm_plt.cuda(non_blocking=True)

        with torch.no_grad():
            seg_pred = model(points, feature, norm_plt, to_categorical(label, num_classes))  # b,n,50

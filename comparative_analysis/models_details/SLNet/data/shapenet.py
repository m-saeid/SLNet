...
class PartNormalDataset(Dataset):
    def __init__(self, npoints=2500, split='train', normalize=False, in_d=3, out_d=16):
        ...
        self.root = os.path.join(BASE_DIR, 'dataset/shapenetcore_partanno_segmentation_benchmark_v0_normal/')
        self.catfile = os.path.join(self.root, 'synsetoffset2category.txt')
        self.cat = {}
        self.normalize = normalize
        self.nape_use = True if out_d is not None else False
        self.nape = NAPE(in_d, out_d) if out_d is not None else None
    ...
    def __getitem__(self, index):
        ...
        if self.nape_use:
            feature = torch.tensor(point_set).unsqueeze(0)  # 1,n,3     1,1024,3
            feature = self.nape(feature).squeeze()        #   n,out_d    1024,16
            feature = feature.numpy()
            return point_set, feature, cls, seg, normal  # 2048,3   2048,16     1,   2048,      2048,3
        else:
            return point_set, cls, seg, normal  # 2048,3   1,   2048,      2048,3

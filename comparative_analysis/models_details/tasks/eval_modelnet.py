...
if __name__ == '__main__':
    ...
    if args.model == 'pointnet':
        ...
        class args_dgcnn:
            ...
            emb_dims = 1024
            dropout = 0.5
        dgcnn = dgcnn_model(args_dgcnn, output_channels=40)
        model = dgcnn

    elif args.model == 'curvenet':
        # curvenet
        from CurveNet.core.models.curvenet_cls import CurveNet as curvenet_moel
        curvenet = curvenet_moel(num_classes=40, k=20, setting='default')
        model = curvenet

    elif args.model[:8] == 'pointmlp':
        if args.model == 'pointmlp_elite':
            # pointmlp-elite
            from pointMLP.classification_ModelNet40.models.pointmlp import pointMLPElite as pointmlp_elite_model
            pointmlp_elite = pointmlp_elite_model()
            model = pointmlp_elite
        elif args.model == 'pointmlp':
            # pointmlp
            from pointMLP.classification_ModelNet40.models.pointmlp import pointMLP as pointmlp_model
        ...
        #SLNet
        from SLNet.utils.cls_modelnet_util import Classification as slnet_model

        if args.model == 'slnet_embed_dim_16':
            slnet_embed_dim_16 = slnet_model(embed=[3, 16, 'no', 'yes', 0.4])
            model = slnet_embed_dim_16
        elif args.model == 'slnet_embed_dim_32':
            slnet_embed_dim_32 = slnet_model(embed=[3, 32, 'no', 'yes', 0.4])

...
def prep_modelnet(url, data_root):
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        data_root = f'{str(BASE_DIR[:-5])}data/modelnet'
        print(data_root)
        # read data
        train_files = glob.glob(os.path.join(f'{data_root}/modelnet40_ply_hdf5_2048', '*train*.h5'))
        test_files = glob.glob(os.path.join(f'{data_root}/modelnet40_ply_hdf5_2048', '*test*.h5'))
